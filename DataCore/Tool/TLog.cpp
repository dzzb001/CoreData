...
{	
	char pszMsg[LOG_BUF_SIZE] = {0};
	va_list ptr; 
	va_start(ptr, pszFmt);
	//vsprintf(pszMsg, pszFmt, ptr); 
	vsnprintf(pszMsg, LOG_BUF_SIZE, pszFmt, ptr);
	va_end(ptr);
...
{
	wchar_t pszMsg[LOG_BUF_SIZE];
	va_list ptr; 
	va_start(ptr, pszFmt);
	vswprintf(pszMsg, pszFmt, ptr); 
	va_end(ptr);

	char szMsg[LOG_BUF_SIZE];
	...
	while ((type = finder.FindNext(strName) != XFileFinder::X_TYPE_NONE))
	{
		...
		else
		{
			if(type != XFileFinder::X_TYPE_NONE)
			{
				...
				{
					std::string strTmp = strName.substr(begin + 1, end - begin - 1);
					XTime t(strTmp);
