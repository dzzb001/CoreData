...
BOOL XFileFinder::FindFirst(const std::string& strSearchPath, uint32 mask/* = X_TYPE_ALL | X_ATTR_ALL*/)
{
	m_mask = mask & (X_TYPE_ALL | X_ATTR_ALL);
	FindClose();
	
	std::string strTmp = strSearchPath;
...
int XFileFinder::FindNext(std::string& strName)
{
...
BOOL XFileUtil::MakeDir(const std::string &strDirPath)
{
...
BOOL XFileUtil::MakeFullDir(const std::string &strDirPath)
{
...
BOOL XFileUtil::RemoveEmptyDir(const std::string &strDirPath)
{
	return RemoveDirectory(strDirPath.c_str());
}

BOOL XFileUtil::RemoveDir(const std::string &strDirPath)
{
	XFileFinder finder;
	std::string strName;
...
BOOL XFileUtil::RemoveFile(const std::string &strFilePath)
{
	return (BOOL)(0 == remove(strFilePath.c_str()));
}

BOOL XFileUtil::MakeLink(const std::string &strTargetPath, const std::string &strNewPath)
{
	return FALSE;
}

BOOL XFileUtil::MakeFullLink(const std::string &strTargetPath, const std::string &strNewPath)
{
	return FALSE;
}

BOOL XFileUtil::IsLink(const std::string &strpath, /*out*/std::string &strTargetPath)
{
	return FALSE;
}

BOOL XFileUtil::IsDir(const std::string &strPath)
{
...
BOOL XFileUtil::IsRegular(const std::string &strPath)
{
	struct _stat statinfo;
	if (0 != ::_stat(strPath.c_str(), &statinfo))
	{
...
BOOL XFileUtil::IsExist(const std::string &strPath)
{
	return (BOOL)(0 == ::_access_s(strPath.c_str(), 00));
}

std::string XFileUtil::GetCurrentDir()
{
	char buf[PATH_MAX] = {};
...
BOOL XFileUtil::SetCurrentDir(const std::string &strDir)
{
	return SetCurrentDirectory(strDir.c_str());
}

BOOL XFileUtil::GetModuleFile(std::string &strDirPath, std::string &strName)
{
	char buf[PATH_MAX];
...
void XFileUtil::ParseFilePath(const std::string &strFilePath, /*out*/std::string &strDirPath, /*out*/std::string &strFileName)
{
	std::vector<std::string> vItems;
...
void XFileUtil::ParseFileName(const std::string &strFileName, /*out*/std::string &strPerfix, /*out*/std::string &strPostfix)
{
	std::string strDirPath, strTmp;
	std::vector<std::string> vItems;
...
std::string XFileUtil::GetSystemDir()
{
	char buf[PATH_MAX];
...
BOOL XFileUtil::MakeFullLink(const std::string &strTargetPath, const std::string &strNewPath)
{
	std::string strDirPath, strFileName, strTmp;
